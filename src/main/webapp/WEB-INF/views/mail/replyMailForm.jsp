...
<body>
	...
	<div class="outer">
		...
		<form id="sendMailForm" method="post" action="send.ml" enctype="multipart/form-data" style="padding:0px 20px;">
			...
			<input type='hidden' name="recipientMail" id="recipientMail" value="${ m.senderMail }">
			...
			<table style="margin:20px 0; font-size:14px;" id="mail-send-form">
				...
				<tr>
					...
					<td class="td">
						<input type="text" name="mailTitle" style="height:30px; border:none;" value="RE: ${ m.mailTitle }">
					</td>
				</tr>
				<tr>
			...
			<script>
				...
				function selectReplyTo(){	// 답장할 멤버 조회, 버튼 추가
					let value = "";
					for(let i in memArr){
						if(memArr[i].mail == "${m.senderMail}"){
							value = "<button class='recipient-btn' type='button'>"
         					 	  + 	memArr[i].name + " " + "&lt" + memArr[i].mail + "&gt"
	           					  ...
	           					  + "</button>";
							arr0.push(memArr[i].mail);
						}
					}
					document.getElementsByClassName("span")[0].innerHTML += value;
					let newWidth = 1000 - document.getElementsByClassName("span")[0].offsetWidth;
					document.getElementsByClassName("mailInput")[0].style.width = newWidth + "px";
				}
